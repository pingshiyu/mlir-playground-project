...
using AddOpLowering = BinaryOpLowering<test::AddOp, arith::AddFOp>;
...
struct ConstantOpLowering : public OpRewritePattern<test::ConstantOp> {
  using OpRewritePattern<test::ConstantOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(test::ConstantOp op,
...
struct FuncOpLowering : public OpConversionPattern<test::FuncOp> {
  using OpConversionPattern<test::FuncOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(test::FuncOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
struct PrintOpLowering : public OpConversionPattern<test::PrintOp> {
  using OpConversionPattern<test::PrintOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(test::PrintOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
struct ReturnOpLowering : public OpRewritePattern<test::ReturnOp> {
  using OpRewritePattern<test::ReturnOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(test::ReturnOp op,
                                PatternRewriter &rewriter) const final {
...
struct TransposeOpLowering : public ConversionPattern {
  TransposeOpLowering(MLIRContext *ctx)
      : ConversionPattern(test::TransposeOp::getOperationName(), 1, ctx) {}
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    lowerOpToLoops(op, operands, rewriter,
                   [loc](OpBuilder &builder, ValueRange memRefOperands,
                     ...
                     test::TransposeOpAdaptor transposeAdaptor(memRefOperands);
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  target.addDynamicallyLegalOp<test::PrintOp>([](test::PrintOp op) {
