//===- MLIRGen.cpp - MLIR Generation from Test AST ------------------------===//
//
...
/// Implementation of a simple MLIR emission from the Test AST.
///
/// This will emit operations that are specific to the Test language, preserving
...
public:
  ...
  /// Public API: convert the AST for a Test module (source file) to an MLIR
  /// Module operation.
  mlir::ModuleOp mlirGen(ModuleAST &moduleAST) {
    ...
    for (auto &record : moduleAST) {
      if (FunctionAST *funcAST = llvm::dyn_cast<FunctionAST>(record.get())) {
        mlir::test::FuncOp func = mlirGen(*funcAST);
        if (!func)
          return nullptr;
...
private:
  /// A "module" matches a Test source file: containing a list of functions.
  ...
  llvm::StringMap<mlir::test::FuncOp> functionMap;
  ...
  /// Helper conversion for a Test AST location to an MLIR location.
  ...
  /// provided Test AST prototype.
  mlir::test::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    return builder.create<mlir::test::FuncOp>(location, proto.getName(),
                                             funcType);
  }

  /// Emit a new function and add it to the MLIR module.
  mlir::test::FuncOp mlirGen(FunctionAST &funcAST) {
    // Create a scope in the symbol table to hold variable declarations.
    SymbolTableScopeT varScope(symbolTable);

    // Create an MLIR function for the given prototype.
    builder.setInsertionPointToEnd(theModule.getBody());
    mlir::test::FuncOp function = mlirGen(*funcAST.getProto());
    if (!function)
      return nullptr;
  ...
  /// data in an Attribute attached to a `test.constant` operation.
  ...
  ///   %0 = "test.constant"() {value: dense<tensor<2x3xf64>,
  ...
  /// other literals in an Attribute attached to a `test.struct_constant`
  ...
  mlir::Value mlirGen(LiteralExprAST &lit) {
    ...
    // Build the MLIR op `test.constant`. This invokes the `ConstantOp::build`
    // method.
 ...
 /// Emit a struct literal. It will be emitted as an array of
  ...
  mlir::Value mlirGen(StructLiteralExprAST &lit) {
    ...
    // Build the MLIR op `test.struct_constant`. This invokes the
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    if (callee == "transpose") {
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: test.transpose "
    ...
    mlir::test::FuncOp calledFunc = calledFuncIt->second;
  ...
  mlir::Value mlirGen(ExprAST &expr) {
    switch (expr.getKind()) {
    case test::ExprAST::Expr_BinOp:
      return mlirGen(cast<BinaryExprAST>(expr));
    case test::ExprAST::Expr_Var:
      return mlirGen(cast<VariableExprAST>(expr));
    case test::ExprAST::Expr_Literal:
      return mlirGen(cast<LiteralExprAST>(expr));
    case test::ExprAST::Expr_StructLiteral:
      return mlirGen(cast<StructLiteralExprAST>(expr));
    case test::ExprAST::Expr_Call:
      return mlirGen(cast<CallExprAST>(expr));
    case test::ExprAST::Expr_Num:
  ...
  /// Build an MLIR type from a Test AST variable type (forward to the generic
  /// getType above).
